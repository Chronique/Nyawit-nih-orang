...
export const TanamView = () => {
  ...
  const fetchApyData = useCallback(async () => {
    ...
    } catch (e) { console.warn("[TanamView] APY fetch failed:", e); }
  }, []);
    ...
    <div className="pb-32 space-y-4">
      ...
      <div className="space-y-3">
        {loading && positions.length === 0 ? (
          <div className="text-center py-12 animate-pulse text-zinc-500 text-xs">Checking Morpho positions...</div>
        ) : (
          MORPHO_VAULTS.map(vault => {
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-3 space-y-1.5">
        ...
        <div className="space-y-1 text-xs text-zinc-500">
          <div className="flex gap-2"><span className="text-green-500 shrink-0">1.</span><span>Sweep dust tokens to WETH in the Sweep tab</span></div>
