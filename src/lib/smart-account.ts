...
import { baseSepolia } from "viem/chains"; // 🔥 GANTI KE SEPOLIA
...
export const getSmartAccountClient = async (walletClient: WalletClient) => {
  ...
  // Custom Owner Wrapper (Untuk EOA / MetaMask)
  const customOwner = {
  ...
  // 5. Setup Smart Account Client (TANPA PAYMASTER)
  return createSmartAccountClient({
    account: coinbaseAccount,
    chain: baseSepolia,
    bundlerTransport: http(PIMLICO_URL),
    
    // 🔥 Paymaster DIHAPUS. User bayar gas sendiri.
